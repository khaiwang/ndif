...
class Processor:
    ...
    Attributes:
        ...
        inflight_request_ids: Set of request IDs currently in flight against
            the model actor. Multiple may be present concurrently because the
            actor batches; capped by ``MAX_INFLIGHT``.
        inflight_started_at: Per-request start timestamp keyed by request ID.
    ...
    def __init__(
        ...
        # wait in self.queue once the semaphore is exhausted. Multiple
        # in-flight requests can be present concurrently (one per
        # mediator slot) so a single ``current_request_*`` pointer is
        # insufficient — track the active set instead.
    ...
    async def execute(self, request: BackendRequestModel) -> None:
        ...
        Note:
            In-flight tracking is handled by ``_run`` (the wrapper that
            spawns this); ``inflight_request_ids`` and
            ``inflight_started_at`` reflect the active set at any given
            time and may contain multiple entries concurrently.

            On actor lookup failure, the processor is cancelled and evicted.
            On other errors, the error is reported via the error_queue.
        """
        ...
        with trace_span("processor.execute", parent_context=parent_ctx) as span:
            ...
            try:
                ...
                # Awaiting here blocks this task until the actor finishes
                # this specific request. With multiple in-flight requests
                # (one per mediator slot) the semaphore in ``_run`` caps
                # fan-out; the actor's batch server fuses concurrent
                # in-flight requests into shared forward passes.
                result = await result
    ...
    async def processor_worker(self) -> None:
        ...
        This is the primary entry point for processor operation. It handles:
            ...
            4. Processing requests from the queue, fanning each out to
               the actor concurrently up to ``MAX_INFLIGHT``
    ...
    def get_state(self) -> dict[str, object]:
        ...
        Returns:
            A dictionary containing:
                ...
                - inflight_request_ids: Sorted list of request IDs in flight.
                - inflight_count: Number of in-flight requests.
                - inflight_started_at: Per-request start timestamps.
    ...
    async def kill_request(self, request_id: str) -> dict:
        ...
        """
        # Check if it's currently in flight (one of potentially many).
